...
export const checkoutSessionCompletedSchema = z.object({
    ...
    subscription: z.string().optional().transform(val => (val ?? undefined) as StripeSubscriptionId | undefined),
    metadata: z.object({
        userId: z.string().min(1, "userId is required").transform(val => val as UserId),
