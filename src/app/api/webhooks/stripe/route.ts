...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        if (error && typeof error === 'object' && ('code' in error || 'message' in error)) {
            ...
            if (err.code === 6 || err.code === 'ALREADY_EXISTS' || err.message?.includes('ALREADY_EXISTS')) {
    ...
    try {
        ...
        if (error instanceof Error && error.name === 'ZodError') {
            ...
            return NextResponse.json({ error: `Webhook validation failed: ${zodError.message}` }, { status: 400 });
        }
...
async function handleCheckoutSessionCompleted(
    ...
    await db.collection('subscriptions').doc(subscription as string).set({
        userId,
        tier,
...
async function handleInvoicePaymentFailed(
    ...
    console.warn(`⚠️ Payment failed for invoice ${invoice.id} (Sub: ${subscription}, Reason: ${billing_reason})`);
